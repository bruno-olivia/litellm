...
class LangFuseLogger:
    ...
    @staticmethod
    def _build_emitted_metadata(
        allowlisted_metadata: StandardLoggingMetadata,
        steering_metadata: Mapping[str, Any],
        enrichments: Mapping[str, Any],
    ) -> dict[str, Any]:  # mutable-ok: the langfuse client mutates the blob it is handed
        """Return the allowlisted metadata, plus caller scalars and litellm enrichments.

        Only scalars are re-injected: a UserAPIKeyAuth object and the nested dicts
        carrying team callback_vars are the non-scalars, so the filter is what keeps
        credentials out.
        """
        assembled: Final = {
            ...
            **{
                ...
                if isinstance(value, (str, int, float, bool)) and key not in allowlisted_metadata
            },
            **enrichments,
        }
        return redact_user_api_key_info(metadata=assembled)
    ...
    def _log_langfuse_v2(
        ...
        try:
            ...
            log_provider_specific_information_as_span(
                trace,
                self._build_emitted_metadata(allowlisted_metadata, clean_metadata, enrichments),
            )
