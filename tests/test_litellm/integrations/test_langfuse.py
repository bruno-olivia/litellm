...
class TestLangfuseUsageDetails(unittest.TestCase):
    ...
    def test_string_customer_metadata_survives_the_migration(self):
        """
        Caller-supplied string keys are re-injected onto the allowlisted blob, so
        moving to StandardLoggingPayload does not silently drop them. Two keys, since
        one cannot distinguish "re-injects every key" from "re-injects the first".
        """
        generation_metadata = self._drive_with_canary()

        assert generation_metadata["first_custom"] == "keep-first"
        assert generation_metadata["second_custom"] == "keep-second"
    ...
    def test_caller_strings_cannot_overwrite_allowlisted_identity_fields(self):
        """
        Re-injection is skipped for keys the allowlisted payload already owns, so a
        caller cannot put user_api_key_alias in request metadata and have their value
        win in the trace. The proxy-resolved identity is what gets emitted.
        """
        generation_metadata = self._drive_with_canary(
